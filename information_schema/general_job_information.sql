...
BEGIN
    WITH src AS (
      SELECT
        ...
        ROUND(COALESCE(total_bytes_billed, 0) / POW(1024, 2), 2) AS totalMebibytesBilled,
        ROUND(COALESCE(total_bytes_billed, 0) / POW(1024, 3), 2) AS totalGigibytesBilled,
        ROUND(COALESCE(total_bytes_billed, 0) / POW(1024, 4), 2) AS totalTebibytesBilled,
